...
def main():
    """Parse command-line arguments, fetch data, and generate plots."""
    parser = argparse.ArgumentParser(
        description="Generate polar scatter plots from the database."
    )
    parser.add_argument("--harm_number", type=int, default=3, help="Harmonic number to filter by.")
    parser.add_argument("--phase", type=str, choices=["Phase A", "Phase B", "Phase C"],
                        default="Phase A", help="Phase to use for plotting.")
    parser.add_argument("--night_mode", action="store_true", help="Enable night mode filtering.")
    parser.add_argument("--threshold", type=float, default=None,
                        help="Threshold percentage for night mode filtering (e.g., 10 for 10%%).")
    args = parser.parse_args()

    # Fetch filtered data from the database.
    df = fetch_data(args.harm_number, args.night_mode, args.threshold)
    # Generate and save the polar scatter plots.
    generate_and_save_plots(df, args.phase, args.harm_number)
